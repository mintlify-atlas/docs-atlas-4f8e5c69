---
title: "Deploy Tradiciones y Sabores with Docker Compose"
...
<Tip>
  On first startup, the FastAPI backend automatically runs `Base.metadata.create_all()` to create all seven database tables (`plato`, `mesa`, `cliente`, `insumo`, `proveedor`, `pedido`, `item_pedido`) and seeds the initial product catalogue with sample dishes. No manual migration step is required.
</Tip>
